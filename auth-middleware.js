...
const ROLE_PERMISSIONS = {
    owner: [PERMISSIONS.READ, PERMISSIONS.WRITE, PERMISSIONS.DELETE, PERMISSIONS.ADMIN, PERMISSIONS.INVITE, PERMISSIONS.MANAGE_TEAM, PERMISSIONS.MANAGE_USERS, PERMISSIONS.VIEW_ANALYTICS, PERMISSIONS.EXPORT_DATA],
    admin: [PERMISSIONS.READ, PERMISSIONS.WRITE, PERMISSIONS.DELETE, PERMISSIONS.INVITE, PERMISSIONS.MANAGE_USERS, PERMISSIONS.VIEW_ANALYTICS, PERMISSIONS.EXPORT_DATA],
    member: [PERMISSIONS.READ, PERMISSIONS.WRITE, PERMISSIONS.VIEW_ANALYTICS],
    viewer: [PERMISSIONS.READ, PERMISSIONS.VIEW_ANALYTICS]
};
...
async function getUserPermissions(userId, teamId) {
    try {
        const result = await db.query(
            'SELECT tm.role, tm.permissions, t.owner_id FROM team_members tm JOIN teams t ON tm.team_id = t.id WHERE tm.user_id = $1 AND tm.team_id = $2 AND tm.is_active = true',
            [userId, teamId]
        );
        ...
        // If user is team owner, they get all permissions
        if (member.owner_id === userId) {
            ...
            if (member.permissions) {
                const customPerms = JSON.parse(member.permissions);
...
// Validate data access for team
async function validateTeamDataAccess(userId, teamId, action = 'read') {
    const permissionMap = {
...
function requirePermission(permission) {
    return async (req, res, next) => {
        ...
        const teamId = req.params.teamId || req.body.teamId || req.query.teamId || req.user.currentTeamId;
        
        if (!teamId) {
            return res.status(400).json({ 
                success: false, 
                message: 'Team ID required for permission check' 
            });
        }
        
        try {
            // Check if user has the required permission for this team
            const hasAccess = await hasPermission(req.user.id, teamId, permission);
            ...
            // Store team ID in request for later use
            req.teamId = teamId;
            next();
        } catch (error) {
...
// Enhanced team access middleware with permission caching
function requireTeamAccessWithPermissions(req, res, next) {
    if (!req.user) {
        return res.status(401).json({ 
            success: false, 
            message: 'Authentication required' 
    ...
    getUserPermissions(req.user.id, teamId)
        .then(permissions => {
