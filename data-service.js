...
class DataService {
  ...
  // Fetch scorecard data for DASHBOARD (from Tableau Public - real-time)
  async getScorecardData() {
    console.log('📈 Fetching dashboard scorecard data from Tableau Public...');
    
    // First, try to get scorecard data from Tableau integration
    try {
      const tableauData = await tableauIntegration.getComprehensivePlatformData();
      if (tableauData && tableauData.scorecard) {
        console.log('✅ Scorecard data fetched from Tableau Public');
        return this.transformTableauScorecardForDashboard(tableauData.scorecard);
      }
    } catch (error) {
      console.log('⚠️ Tableau scorecard data not available:', error.message);
    }
    ...
    return this.transformTableauScorecardForDashboard({
      customerSatisfaction: 92,
      teamEfficiency: 88,
      goalCompletion: 75,
      qualityScore: 94,
      source: 'tableau_fallback'
    });
  }
