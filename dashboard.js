...
function switchToSection(sectionName) {
    ...
    switch(sectionName) {
        case 'dashboard':
            ...
            break;
        case 'scorecard':
            // Show only scorecard and platform performance
            showSections(['platform-performance', 'scorecard']);
            break;
        case 'rocks':
        case 'todos':
        case 'issues':
            // Show only Ninety.io data section
            showSections(['ninety-data']);
            // Switch to appropriate tab
            switchNinetyTab(sectionName);
            break;
        case 'google-ads':
        case 'facebook-ads':
            // Show only platform performance
            showSections(['platform-performance']);
            break;
        case 'analytics':
            // Show funnel and comparison sections
            showSections(['funnel', 'comparison']);
            break;
        case 'vision':
        case 'org-chart':
        case 'knowledge':
            // Show knowledge base section
            showSections(['knowledge-base']);
            break;
        default:
            // Show all sections
            sections.forEach(section => {
...
function showSections(sectionIds) {
    const sections = document.querySelectorAll('.dashboard-section');
    sections.forEach(section => {
        const shouldShow = sectionIds.some(id => 
            section.innerHTML.includes(id) || 
            section.classList.contains(id) ||
            section.querySelector(`[class*="${id}"]`)
        );
