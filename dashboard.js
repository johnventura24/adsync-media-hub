...
function updateNinetyDisplay(dataType, items) {
    ...
    // Only add filter if we have the original data (not filtered data)
    if (items === currentNinetyData[dataType]) {
...
function addDepartmentFilter(dataType) {
    ...
    // Only show filter if there are multiple departments (more than just "All Departments" + 1 department)
    if (departments.length <= 2) {
        console.log(`Not enough departments for ${dataType} to show filter`);
        return;
    }
