...
// Get available departments from current data (NEW)
function getAvailableDepartments(dataType) {
    const data = currentNinetyData[dataType] || [];
    const departments = [...new Set(data.map(item => item.department || item.Department || 'General').filter(Boolean))];
    return ['All Departments', ...departments.sort()];
}

// Filter data by department (NEW)
function filterDataByDepartment(dataType, department) {
    ...
    return data.filter(item => 
        (item.department || item.Department || 'General') === department
    );
}
...
// Add department filter dropdown (NEW)
function addDepartmentFilter(dataType) {
    ...
    filterDiv.innerHTML = `
        <select class="form-select" onchange="filterByDepartment('${dataType}', this.value)">
