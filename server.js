...
// NEW: Get available dates for dropdown
app.get('/api/available-dates', async (req, res) => {
  try {
    const history = await dailyDataTracker.getDailyHistory(30); // Get last 30 days
    const dates = [];
    
    // Add Google dates
    if (history.google && history.google.length > 0) {
      history.google.forEach(day => {
        if (day.date && !dates.includes(day.date)) {
          dates.push(day.date);
        }
      });
    }
...
// NEW: Get data for specific date
